...
class MyFrame(wx.Frame):
    ...
    def init_data(self):
        ...
        self.oodsBetterThanMeds = np.zeros(len(self.tags))
        for i in range(len(self.varAcc2)):
            for j in range(max(0, i-OODS_WINDOW//2), min(i+OODS_WINDOW//2, len(self.varAcc2))):
                # window = self.varAcc2[max(0, i-OODS_WINDOW//2):(i+OODS_WINDOW//2)]
                if self.varAcc2[j]>LIGHT_THRESH:
                    self.oodsBetterThanMeds[i]=1
                    break
    ...
    def init_plot(self):
        ...
        self.plot_oods = \
            self.bottPlot.plot(self.t[self.i_start:self.i_end],
                               self.oodsBetterThanMeds[self.i_start:self.i_end], 'g', label='oods')[0]
        
        self.bottPlot.legend([self.plot_tags, self.plot_tags2, self.plot_oods])
    ...
    def draw_plot(self):
        ...
        self.plot_oods.set_xdata(self.t[self.i_start:self.i_end])
        ...
        self.plot_oods.set_ydata(self.oodsBetterThanMeds[self.i_start:self.i_end])
